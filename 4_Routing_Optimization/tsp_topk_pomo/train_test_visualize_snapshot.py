...
def visualize(df, result, args):
    ...
    OFF_BLACK = "#111827"
    COOL_GRAY_BG = "#F8FAFC"
    COOL_GRAY_PANEL = "#F1F5F9"
    COOL_GRAY_LINE = "#CBD5E1"
    SAT_BLUE = "#4F46E5"
    LIGHT_BLUE = "#DBEAFE"

    # Blue score colormap: low score = cool gray, high score = saturated blue
    blue_score_cmap = LinearSegmentedColormap.from_list(
        "aeris_blue_score",
        [COOL_GRAY_PANEL, LIGHT_BLUE, SAT_BLUE],
    )

    fig, ax = plt.subplots(figsize=(10, 8), facecolor=COOL_GRAY_BG)
    ax.set_facecolor(COOL_GRAY_BG)

    sc = ax.scatter(
        x,
        y,
        c=s,
        cmap=blue_score_cmap,
    ...
    cbar.set_label("score", color=OFF_BLACK)
    cbar.ax.tick_params(colors=OFF_BLACK)
    cbar.outline.set_edgecolor(COOL_GRAY_LINE)
    ...
    ax.set_title(
        f"Synthetic One-Snapshot POMO TSP | best distance = {result['best_distance']:.4f}",
        color=OFF_BLACK,
        fontweight="bold",
    )
    ...
    for spine in ax.spines.values():
        spine.set_color(COOL_GRAY_LINE)

    ax.grid(True, color=COOL_GRAY_LINE, alpha=0.45, linewidth=0.8)

   

    fig.tight_layout()

    # 300 dpi for poster/print
